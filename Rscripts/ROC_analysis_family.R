...
fig_dir = "/Volumes/qtran/Semisupervised_Learning/figures/family_results/"

model = c('setred_OneNN','snnrce_OneNN', 'selfTraining_OneNN')
m = ""
toutput = read.csv(paste0(main_dir, 'Calibrated_Trans_scores_MCF_wide.csv'), header=TRUE)

toutput_all = read.csv(paste0(main_dir, 'Calibrated_Trans_scores_MCF_long.csv'), header=TRUE)
...
######RAW MAX SCORES########
max_tscore = read.csv(paste0(main_dir, "All_MCF_Transductive_MaxScores_SETRED_SVM.csv"))
max_tscore$match = ifelse(max_tscore$pred_label == max_tscore$truth_label, 1, 0)

#####MAX CALIBRATED SCORES######
toutput$max_score = apply(toutput[, 1:75], 1, max)
toutput$max_index = apply(toutput[,1:75], 1, function(x) which.max(x))
...
pdf(paste0(fig_dir, "/ROC_YoudenIndex_Family.pdf"), width=9, height = 9)
...
plot(rocobj_calb, print.thres="best", print.thres.best.method="youden", col="blue", lwd=3, 
     ...
     print.thres.col="blue", print.thres.adj=c(0.12,-1.6), print.thres.best.weights=c(1, 0.5), 
     print.auc=TRUE, print.auc.y = 0.945, print.auc.x = 0.85, main="SETRED-SVM",
     cex.lab=2, cex.axis=2)
plot(rocobj_raw, print.thres="best", print.thres.best.method="youden", col="red", lwd=3, 
     auc.polygon.col="lightpink",print.thres.adj=c(0.15,2),
     print.thres.col="red", print.auc=TRUE, add=TRUE, print.auc.y = 0.85, print.auc.x = 0.85)
legend(x=0.25,y= 0.1, legend=c("raw","calibrated"), lty = c(1,1), col=c("red","blue"), lwd=c(2,2))
dev.off()


pdf(paste0(fig_dir, "Threshold_Prec_Rec_Spec_Family.pdf"), width=9, height = 9)
op=par(cex=1.5, lwd=3)
plot(precision ~ threshold , thres_calb, type = "l", 
     subset = is.finite(threshold), col = "blue", lwd=3,
     cex.lab=1.5, cex.axis=1.5, ylab="Precision/Recall/Specificity", ylim = c(0, 1))
lines(recall ~ threshold, thres_calb, type = "l", lty=2, 
      subset = is.finite(threshold), col = "blue", lwd=3)
lines(specificity ~ threshold, thres_calb, type = "l", lty=3, 
      subset = is.finite(threshold), col = "blue", lwd=3)
abline(v = 0.68, col="black", lwd=3, lty=2)
text(0.71, 1, "prec = 0.999", cex = 1.1, col = "black", adj=c(1,0))
text(0.6, 0.89, "recall = 0.894", cex = 1.1, col = "black", adj=c(0.67,0))
text(0.6, 0.94, "spec = 0.962", cex = 1.1, col = "black", adj=c(0.67,0))
legend(x=0.2, y=0.18, legend=c("precision","recall", "specificity"),
       lty = c(1,2,3), col=c("blue","blue", "blue"), lwd=c(2,2,2))
abline(v = 0.8, col="#b7702d", lwd=3, lty=2)
text(0.86, 0.93, "spec = 0.959", cex = 1.1, col = "#b7702d", adj=c(0.3,0))
text(0.86, 0.785, "rec = 0.798", cex = 1.1, col = "#b7702d", adj=c(0.3,0))
text(0.86, 1.0, "prec = 0.999", cex = 1.1, col = "#b7702d", adj=c(0.3,0))
...
text(0.68, 0.9, "0.718 (0.927, 0.905)", cex = 1, col = "black", adj=c(-0.1,0))
