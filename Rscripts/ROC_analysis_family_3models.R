...
fig_dir = "/Volumes/qtran/Semisupervised_Learning_big_private_data/figures/family_results/"
...
toutput_all = read.csv(paste0(main_dir, 'Calibrated_', m, '_Trans_scores_MCF_long.csv'), header=TRUE)
...
pdf(paste0(fig_dir, "/ROC_YoudenIndex_", m, "_Family.pdf"), width=9, height = 9)
...
plot(rocobj_calb, print.thres="best", print.thres.best.method="youden", col="blue", lwd=3, 
     ...
     print.auc=TRUE, print.auc.y = 0.89, print.auc.x = 0.88, cex.sub = 2, main=m,
     cex.lab=2, cex.axis=2)
plot(rocobj_raw, print.thres="best", print.thres.best.method="youden", col="red", lwd=3, 
     auc.polygon.col="lightpink", cex.sub=2,
     print.thres.col="red", print.auc=TRUE, add=TRUE, print.auc.y = 0.68, print.auc.x = 0.80)
legend(x=0.25,y= 0.1, legend=c("raw","calibrated"), lty = c(1,1), col=c("red","blue"), lwd=c(3,3),pt.cex=1)
dev.off()


pdf(paste0(fig_dir, "Threshold_Prec_Rec_Spec_", m, "_Family.pdf"), width=8, height = 8)
...
lines(specificity ~ threshold, thres_calb, type = "l", lty=3, 
      subset = is.finite(threshold), col = "blue", lwd=2)
abline(v = 0.525, col="black", lwd=2, lty=2)
...
pdf(paste0(fig_dir, "/Threshold_vs_", m, "_Spec+Sens_Family.pdf"))
...
pdf(paste0(fig_dir, "/Threshold_calibrated_vs_Spec_Sens_", m, "_Family.pdf"))
...
pdf(paste0(fig_dir, "/Prec_vs_Recall_", m, "_Family.pdf"))
plot(precision ~ threshold , thres_calb, type = "l", 
     subset = is.finite(threshold), col = "blue", lwd=2,
...
write.csv(thres_class, file = paste0(base_dir, "/Threshold_calibrated_", m, "_scores_perMCF.csv"))
...
#lines(x=c(0,1), c(0,1))
pdf(paste0(fig_dir, "Raw_vs_Calibrated_Max_Scores_", m, "_Family.pdf"))
