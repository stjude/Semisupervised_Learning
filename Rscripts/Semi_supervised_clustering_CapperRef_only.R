...
table(round(sd.vals,2) >= 0.3)
new_beta = top_beta[1:5072, ]

###unlabeled data
label_samples = label_data$Sample
label_data = new_beta[, (colnames(new_beta) %in% label_samples)]
label_data_x = t(label_data)

data_y = tsne_data[match(rownames(label_data_x), tsne_data$Sample), c("Sample", "meth.family")]
#rownames(label_data_y) = label_data_y$Sample
label_data_y = data_y[ , title.class.x]
