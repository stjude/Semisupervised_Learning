...
top_beta = fread("/research/rgs01/home/clusterHome/qtran/Capper_reference_set/top_32K_GSE90496.txt")
labels = read.csv("/research/rgs01/home/clusterHome/qtran/Capper_reference_set/GSE90496_MC_MCF_color_labels_key.csv")

#family_label = read.csv("/Volumes/qtran/Capper_reference_set/German_subClass_family_labels.csv")
top_beta = fread("/Volumes/qtran/Capper_reference_set/top32K_GSE90496.txt")
#labels = read.csv("/Volumes/qtran/Capper_reference_set/GSE90496_MC_MCF_color_labels_key.csv")
...
###We use 4060 probes that have the SD > 0.3
sd.vals = apply(top_beta, 1, sd)
table(round(sd.vals,2) >= 0.3) #### ---> provide 5027 probes
new_beta = top_beta[1:5072, ]

###unlabeled data
label_samples = labels$Sample
label_data = new_beta[, (colnames(new_beta) %in% label_samples)]
label_data_x = t(label_data)

data_y = labels[match(rownames(label_data_x), labels$Sample), c("Sample", "meth.family")]
#rownames(label_data_y) = label_data_y$Sample
label_data_y = data_y[ , 'meth.family']
names(label_data_y) = data_y$Sample
...
base_dir = "/research/rgs01/home/clusterHome/qtran/Semisupervised_learning/"
...
for (i in m){
  ...
  write.csv(acc, file =paste0(base_dir, "processed_data/ACC_SNNRCE.csv"))
  ...
  write.csv(i_label_data, file=paste0(base_dir, "processed_data/Inductive_predicted_SNNRCE_labels.csv"))
  write.csv(t_label_data, file=paste0(base_dir, "processed_data/Transductive_predicted_SNNRCE_labels.csv"))
  
  ###get predicted probabilities
  print(paste("j=",j))
}

write.csv(acc, file =paste0(base_dir, "processed_data/ACC.csv"))
