...
load("/Volumes/qtran/Semisupervised_Learning/best.setred.svm.family.RData")

sorted_xitest = read.table("/rgs01/home/clusterHome/qtran/Semisupervised_Learning/raw_data/GSE109379_top5K_sorted_input.txt", header=TRUE)
gse_label = read.csv("/rgs01/home/clusterHome/qtran/Semisupervised_Learning/processed_data/GSE109379_old_vs_selfKNN_labels.csv", header=TRUE)
...
load("/Volumes/qtran/Semisupervised_Learning/calibration_model_family.RData")
best.lambda = read.table("/Volumes/qtran/Semisupervised_Learning/lambda_for_calibration_model_family.txt")

gse_calb = predict(fit, pred_iscore, s = best.lambda$x, type = "response")
gse_calb = as.data.frame(gse_calb)
pred_class = as.data.frame(pred_class)
rownames(pred_class) = rownames(sorted_xitest)
pred_class$pred_class = trimws(pred_class$pred_class)
#gse_label$setred.label = pred_class$pred_class[match(gse_label$GSM.ID, substr(rownames(pred_class), 0, 10))]
gse_label$setred.label = pred_class$pred_class[match(gse_label$Samples, rownames(pred_class))]
...
write.csv(gse_label, "/rgs01/home/clusterHome/qtran/Semisupervised_Learning/processed_data/GSE109379_old_vs_setred_family_labels.csv")
write.csv(output, "/rgs01/home/clusterHome/qtran/Semisupervised_Learning/processed_data/GSE109379_setred_output.csv")

pred_class$max_score = max_score
write.csv(output, "/Volumes/qtran/Semisupervised_Learning/processed_data/GSE109379_setred_output_family_24Jan2022.csv")
write.csv(gse_label, "/Volumes/qtran/Semisupervised_Learning/processed_data/GSE109379_old_vs_setred_family_24Jan2022.csv")
