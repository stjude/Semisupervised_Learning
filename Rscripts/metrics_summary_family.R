...
#base_dir = "/Volumes/qtran/Semisupervised_Learning/processed_data/family_results"
...
ind_data$MCF = gsub("Class: ", "", ind_data$MCF)
trans_data$MCF = gsub("Class: ", "", trans_data$MCF)

write.csv(ind_data, file = paste0(base_dir, "/All_MCF_Inductive_metrics.csv"), row.names=FALSE)
write.csv(trans_data, file = paste0(base_dir, "/All_MCF_Transductive_metrics.csv"), row.names=FALSE)

colnames(ind_score)[1] = "Sample"
colnames(trans_score)[1] = "Sample"
library(tidyr)
ind_score_long <- gather(ind_score, pred_family, pred_score, 2:76, factor_key=FALSE)
trans_score_long <- gather(trans_score, pred_family, pred_score, 2:76, factor_key=FALSE)

ind_score_long$pred_family = gsub("\\.\\.", ", ", ind_score_long$pred_family)
ind_score_long$pred_family = gsub(".", " ", ind_score_long$pred_family, fixed=TRUE)
ind_score_long$match = ifelse(ind_score_long$truth_label == ind_score_long$pred_family, 1, 0)

trans_score_long$pred_family = gsub("\\.\\.", ", ", trans_score_long$pred_family)
trans_score_long$pred_family = gsub(".", " ", trans_score_long$pred_family, fixed=TRUE)
trans_score_long$match = ifelse(trans_score_long$truth_label == trans_score_long$pred_family, 1, 0)

write.csv(ind_score_long, file = paste0(base_dir, "/All_MCF_SETRED_SVM_Inductive_scores_long.csv"), row.names=FALSE)
write.csv(trans_score_long, file = paste0(base_dir, "/All_MCF_SETRED_SVM_Transductive_scores_long.csv"), row.names=FALSE)

write.csv(ind_score, file = paste0(base_dir, "/All_MCF_SETRED_SVM_Inductive_scores_wide.csv"), row.names=FALSE)
write.csv(trans_score, file = paste0(base_dir, "/All_MCF_SETRED_SVM_Transductive_scores_wide.csv"), row.names=FALSE)
...
write.csv(max_iscore, file = paste0(base_dir, "/All_MCF_Inductive_MaxScores_SETRED_SVM.csv"), row.names=FALSE)
write.csv(max_tscore, file = paste0(base_dir, "/All_MCF_Transductive_MaxScores_SETRED_SVM.csv"), row.names=FALSE)
