...
predClass.OneNN <- function(m, dists) {
  ...
  if(ncol(dists) != length(m$model$y)){
...
predProb.OneNN <- function(m, dists, distance.weighting = "reciprocalexp", initial.value = 0) {
  ...
  probabilities <- matrix(data = initial.value, nrow = nrow(dists), ncol = length(levels(m$model$y)))
  minDistances <- matrix(nrow = nrow(dists), ncol = length(levels(m$model$y)))
  
  for (q in 1:nrow(dists)) { # for each query instance
    # find the most close instance to xi  for each class in dists
    for (j in 1:length(m$model$y)) {
      cj <- unclass(m$model$y[j]) # clase
  ...
  colnames(probabilities) <- levels(m$model$y)
  probabilities
}
...
predict.OneNN <- function(object, dists, type=c("prob", "class"), ...){
  if(type == "class"){
