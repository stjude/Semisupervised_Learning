...
# Define the control
control <- trainControl(method = "repeatedcv",number = 3,search = "grid", repeats=5, classProbs = TRUE,verboseIter = TRUE,
...
save(rf1, file = "./processed_data/class_results/rf1_seed160_class.RData" )
tClasses1 <- predict(rf1, newdata = xitest)
yitest = gsub(", ", "..", yitest)
yitest = gsub(" ", ".", yitest)
predict1 = confusionMatrix(tClasses1, as.factor(yitest))
write.table(tClasses1, file = "./processed_data/class_results/rf1_predicted_label_seed160.txt")
save(predict1, file = "./processed_data/class_results/rf1_seed160_Ind_CM.RData")

########### DRAW confusion matrix for
#load("./processed_data/class_results/seed1600_rf1_confusionMatrix.RData")
library(cvms)
...
save(eval1, file = "./processed_data/class_results/rf1_seed160_itest_eval.RData")
...
plot_cm(eval1, filename = "./figures/class_results/rf1_seed160_ind_testing_CM.pdf", color_key)
#rs <- rf.roc[['rocs']]
#plot.roc(rs[[1]])
...
####select the score from seed1600 which yielded the best SETRED-SVM
yttest_seed160 = yttest[yttest$seed == "seed160_all", ]
###get the label with max scores
#aggregate(pred_score ~ Sample, data=yttest_seed1600, max)
labelseed160 = merge(aggregate(pred_score ~ Sample, data=yttest_seed160, max), yttest_seed160, all.x=T)
...
comb_data$ytrain[which(is.na(comb_data$ytrain))] = 
  labelseed160$pred_label[match(rownames(comb_data)[which(is.na(comb_data$ytrain))] ,labelseed160$Sample)]
...
save(rf2, file = "./processed_data/class_results/rf2_seed160_add35Train_class.RData")
