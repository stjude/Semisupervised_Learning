...
load("./processed_data/class_results/seed80_alldata_sets.RData")
...
save(rf1, file = "./processed_data/class_results/rf1_seed80_class.RData" )
tClasses1 <- predict(rf1, newdata = xitest) ###just get the labels
yitest = gsub(", ", "..", yitest)
yitest = gsub(" ", ".", yitest)
predict1 = confusionMatrix(tClasses1, as.factor(yitest))
write.table(tClasses1, file = "./processed_data/class_results/rf1_predicted_label_seed80.txt")
save(predict1, file = "./processed_data/class_results/rf1_seed80_Ind_CM.RData")
...
tClasses1 <- predict(rf1, newdata = xitest, type='prob') ###get the probability for each class

eval1 = get_perm_metric(yitest, tClasses1)
save(eval1, file = "./processed_data/class_results/rf1_seed80_itest_eval.RData")
...
plot_cm(eval1, filename = "./figures/class_results/rf1_seed80_ind_testing_CM.pdf", color_key)
#rs <- rf.roc[['rocs']]
#plot.roc(rs[[1]])
...
yttest_seed80 = yttest[yttest$seed == "seed80_all", ]
###get the label with max scores
#aggregate(pred_score ~ Sample, data=yttest_seed800, max)
labelseed80 = merge(aggregate(pred_score ~ Sample, data=yttest_seed80, max), yttest_seed80, all.x=T)
...
comb_data$ytrain[which(is.na(comb_data$ytrain))] = 
  labelseed80$pred_label[match(rownames(comb_data)[which(is.na(comb_data$ytrain))] ,labelseed80$Sample)]
...
save(rf2, file = "./processed_data/class_results/rf2_seed80_add35Train_class.RData")
...
write.table(tClasses2, file = "./processed_data/class_results/rf2_predicted_label_seed80.txt")
save(predict2, file = "./processed_data/class_results/rf2_seed80_ind_CM.RData")
...
save(eval2, file = "./processed_data/class_results/rf2_seed80_itest_eval.RData")

#color_key = read.csv("/Volumes/qtran/Capper_reference_set/color.key.allRef.csv")

plot_cm(eval2, filename = "./figures/class_results/rf2_seed80_ind_testing_CM.pdf", color_key)
