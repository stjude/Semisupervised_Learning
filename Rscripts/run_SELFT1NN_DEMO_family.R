...
for (s in c(1)){
  ...
  set.seed(3)
  source("/rgs01/home/clusterHome/qtran/Semisupervised_Learning/Rscripts/triTrain.R")
  m.trit <- triTraining(x = dtrain, y = ytrain, x.inst=FALSE,
                        learner = ssc::oneNN,  
                        pred = "predict", 
                        pred.pars = NULL)
...
####Prediction###3
  m = list(m.selft1,  m.setred,  m.snnrce, m.trit,  m.demo)
  names(m) = c("m.selft1", "m.setred",  "m.snnrce", "m.trit", "m.demo")
  rlist::list.save(m, file = paste0(output_tbl_dir, 'models_next_top_5.RData'))
  ...
  for (i in m){
    ...
    if(trainer == "triTraining"){
      ditest <- as.matrix(proxy::dist(ditest[, i$instance.index], method = "euclidean", by_rows = TRUE))
      ipred_score = predProb.OneNN(i$model[[1]], ditest[, i$instance.index])
      rownames(ipred_score) = rownames(ditest)
      
      dttest <- as.matrix(proxy::dist(x = xttest, y = xtrain[i$instances.index,], method = "euclidean", by_rows = TRUE))
      tpred_score = predProb(i, dttest)
      rownames(tpred_score) = rownames(dttest)
      write.csv(ipred_score, file=paste0(output_tbl_dir,"/Inductive_scores_", trainer, "_", learner, ".csv"), row.names=TRUE)
      write.csv(tpred_score, file = paste0(output_tbl_dir,"/Transductive_scores_", trainer, "_", learner, ".csv"), row.names=TRUE)
    ...
    if(trainer =="democratic"){
      ipred_score = predProb.demo(i, ditest[, i$instances.index], pred.pars = list(NULL, list(type = "probabilities"), list(type = "prob")),
                                  pred = predict)
      tpred_score = predProb.demo(i, dttest[, i$instances.index],pred = predict, 
                                  pred.pars =list(NULL, list(type = "probabilities"), list(type = "prob")))
      write.csv(ipred_score, file=paste0(output_tbl_dir,"/Inductive_scores_", trainer,  ".csv"), row.names=TRUE)
      write.csv(tpred_score, file = paste0(output_tbl_dir,"/Transductive_scores_", trainer, ".csv"), row.names=TRUE)
      
      ioutscores = create_class_score_output(ipred, ipred_score, yitest)
      toutscores = create_class_score_output(transpred, tpred_score, yttest)
      write.csv(ioutscores, file=paste0(output_tbl_dir,"/Inductive_MaxScores_", trainer, ".csv"), row.names=TRUE)
      write.csv(toutscores, file = paste0(output_tbl_dir,"/Transductive_MaxScores_", trainer,  ".csv"), row.names=TRUE)
    }
  ...
  write.csv(i_label_data, file=paste0(output_tbl_dir, "/Inductive_predicted_ALL_labels.csv"), row.names=TRUE)
  write.csv(t_label_data, file=paste0(output_tbl_dir, "/Transductive_predicted_ALL_labels.csv"), row.names=TRUE)
