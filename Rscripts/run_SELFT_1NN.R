...
for (s in c(50)){
  ...
  rlist::list.save(m, file = 'models_SELFT.RData')
  j = 0
  ...
  for (i in m){
    ...
    iprob = predict.OneNN (m.self1, ditest[, m.self1$instances.index], type="prob")
    tprob = predict.OneNN (m.self1, ditest[, m.self1$instances.index], type="prob")
  ...
  write.csv(acc, file =paste0(output_tbl_dir, "/ACC_SELFT.csv"))
  write.csv(i_label_data, file=paste0(output_tbl_dir, "/Inductive_predicted_SELFT_labels.csv"))
  write.csv(t_label_data, file=paste0(output_tbl_dir, "/Transductive_predicted_SELFT_labels.csv"))
