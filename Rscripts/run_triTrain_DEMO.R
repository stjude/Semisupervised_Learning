...
####Prediction###3
  m = list(m.trit,  m.demo)
  names(m) = c("m.trit", "m.demo")
  rlist::list.save(m, file = paste0(output_tbl_dir, 'models_triT_DEMO.RData'))
  ...
  write.csv(i_label_data, file=paste0(output_tbl_dir, "/Inductive_predicted_TRITRAIN_DEMO_labels.csv"), row.names=TRUE)
  write.csv(t_label_data, file=paste0(output_tbl_dir, "/Transductive_predicted_TRITRAIN_DEMO_labels.csv"), row.names=TRUE)
