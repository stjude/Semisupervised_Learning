...
for (s in c(25)){
  print(paste0("Strat with seed = ", s))
  ...
  m = list(m.trit, m.trit2, m.trit3)
  names(m) = c("m.tritKNN", "m.tritSVM", "m.tritC50")
  rlist::list.save(m, file = 'modelsTRIT.RData')
  j = 0
  ...
  for (i in m){
    ...
    write.csv(acc, file =paste0(output_tbl_dir, "/ACC_TRITRAIN.csv"))
    ...
    #write.csv(i_label_data, file=paste0(output_dir, "processed_data/Inductive_predicted_TRITRAIN_labels.csv"))
    #write.csv(t_label_data, file=paste0(output_dir, "processed_data/Transductive_predicted_TRITRAIN_labels.csv"))
  ...
  write.csv(acc, file =paste0(output_tbl_dir, "/ACC_TRITRAIN.csv"))
  write.csv(i_label_data, file=paste0(output_tbl_dir, "/Inductive_predicted_TRITRAIN_labels.csv"))
  write.csv(t_label_data, file=paste0(output_tbl_dir, "/Transductive_predicted_TRITRAIN_labels.csv"))
